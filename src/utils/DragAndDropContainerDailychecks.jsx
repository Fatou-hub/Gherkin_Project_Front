...
//   return (
//     <>
//       <h4 id="DnD_title">Welcome to Dailychecks Page</h4>
//       <div id="DND_BTN">
...
import Container from "../utils/container";
import { Item } from '../utils/sortable_items';
import { Navigate } from 'react-router-dom';  // Import de Navigate
...
export default function DragAndDropContainerDailychecks({ pathFilter }) {
  const [items, setItems] = useState({ root: [], container1: [] });
  const [activeId, setActiveId] = useState();
  ...
  function handleSaveAndRedirect() {
    ...
    // Définir l'état de redirection à vrai pour déclencher <Navigate />
    setRedirect(true);
  }

  // Si l'état de redirection est vrai, retourner <Navigate /> pour rediriger
  if (redirect) {
  ...
  return (
    <>
      ...
      <div id="DND_BTN">
        ...
        <div className="button-container">
          <button onClick={handleSaveAndRedirect}>Envoyer</button>
        </div>
      </div>
    </>
  );
