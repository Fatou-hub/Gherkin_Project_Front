...
export default function DragAndDropcreation({ pathFilter }) {
  const [items, setItems] = useState({ root: [], container1: [] });
  const [activeId, setActiveId] = useState();
  const [newItemContent, setNewItemContent] = useState(""); // État pour le nouveau contenu de l'élément
  ...
  const handleNewItemSubmit = (e) => {
    ...
    if (newItemContent.trim() !== "") {
      const newItem = {
        id: Date.now().toString(), // Génère un identifiant unique pour l'élément
      ...
      setNewItemContent(""); // Réinitialise le champ d'entrée
    }
  };
